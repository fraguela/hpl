...
void f1(Array<float, 1> v1, Array<float, 1> v2) 
{
  ...
  for(std::size_t i = 0; i <= 10; ++i)
    reduce(v1[0], v2[idx*8], "+").ndims(1).groupSize(i).nElems(8);
  
  reduce(v1[0], v2[idx*8], "+").ndims(1).groupSize(16).nElems(8).toAll();
  
  reduce(v1[0], v2[idx*8], "+").ndims(1).groupSize(32).nElems(8).toAll();
...
{ Float f;
  ...
  for(std::size_t i = 0; i <= 10; ++i)
    reduce(v1[0], v1[idx*8], "fmax").ndims(1).groupSize(i).nElems(8);
  
  reduce(f, v1[idx*8], "fmax").ndims(1).groupSize(16).nElems(8);
  
  reduce(f, v1[idx*8], "fmax").ndims(1).groupSize(32).nElems(8);
...
{ Float f;
  ...
  reduce(f, v1[idx*8], "+").nElems(8);
    
  reduce(f, v1[idx*8], "+").nElems(8).toAll();
  
  reduce(f, v1[idx*8], "+").ndims(2).nElems(8);
  
  reduce(f, v1[idx*8], "+").ndims(2).groupSize(16).nElems(8);
    
  reduce(f, v1[idx*8], "+").ndims(2).localMem(64).nElems(8);
  
  reduce(f, v1[idx*8], "+").ndims(2).minGroupSize(16).nElems(8);
  
  reduce(f, v1[idx*8], "+").ndims(2).minGroupSize(16).localMem(16).nElems(8);
  
  reduce(f, v1[idx*8], "+").ndims(2).minGroupSize(16).localMem(32).nElems(8);
}
...
{ Array<float, 1, Local> mx(Witems * 4);
  ...
  reduce(partialSums[gidx*4], mx[4*lidx], "+").ndims(1).groupSize(Witems).nElems(4); 
}
...
{ 
  ...
  reduce(partialSums[gidx*4], v3[4*idx], "+").ndims(1).nElems(4); 
}
